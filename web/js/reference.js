 ...
 *
 * Derived from the record's serial and the year of inspection so it is stable,
 * sortable and unique without a second write after insert.
 */
export function certificateReference(serial, inspectionDate) {
...
export function certificateFilename(reference, address) {
  const slug = address
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
